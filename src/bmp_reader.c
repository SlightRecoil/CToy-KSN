...
uint16_t readWORD(uint8_t *data, size_t offset, size_t size) {
    // little endian // This is little-endian because BMP files store the least significant byte first.
...
// HW
// # ask if i was supposed to use struct
...
void assignHead(BITMAPFILEHEADER *zag, uint8_t *data, size_t size) {
    ...
    zag->bfType = readWORD(data, 0, size);
    zag->bfSize = readDWORD(data, 2, size);
    zag->bfReserved1 = readWORD(data, 6, size);
    zag->bfReserved2 = readWORD(data, 8, size);
    zag->bfOffBits = readDWORD(data, 10, size);
}
